...
model_des_on_asc <- function(data, ROI_name, smt = 1) {
  data <- data %>%
    filter(ROI == ROI_name)
  ...
  for (i in seq_along(asc_TRs)) {
    ...
    models[[i]] <- lm(as.formula(str_c("D",i,"~poly(","A",i,",",smt,")")), data = data_model)
  }
...
vis_des_on_asc <- function(data, ROI_name, mods = NULL, smt = 1) {
  data <- data %>%
    filter(ROI == ROI_name)
  ...
  for (i in seq_along(asc_TRs)) {
    ...
    plots[[i]] <- data_model %>%
      ...
      geom_point(size = 1.5) +
      geom_smooth(method = "lm", formula = y~poly(x,smt), se = FALSE, size = 2.5) +
      labs(
        x = str_c("A",i),
        y = str_c("D",i)
      ) +
...
for (i in seq_along(ROIs)) {
  model_ROIs[[i]] <- model_des_on_asc(model_data, ROIs[i], smt[i])
}

# Visualize the BOLD Descent on Ascent for the different ROIs

# VC Visualization
plot_VC <- ggarrange(plotlist = vis_des_on_asc(model_data, ROIs[1], model_ROIs[[1]], smt[1]), ncol = 3, nrow = 2)
annotate_figure(plot_VC, top = text_grob("VC BOLD Descent on Ascent", face = "bold", size = 16))

# l_OFC Visualization
plot_l_OFC <- ggarrange(plotlist = vis_des_on_asc(model_data, ROIs[2], model_ROIs[[2]], smt[2]), ncol = 3, nrow = 2)
annotate_figure(plot_l_OFC, top = text_grob("l_OFC BOLD Descent on Ascent", face = "bold", size = 16))

# m_OFC Visualization
plot_m_OFC <- ggarrange(plotlist = vis_des_on_asc(model_data, ROIs[3], model_ROIs[[3]], smt[3]), ncol = 3, nrow = 2)
